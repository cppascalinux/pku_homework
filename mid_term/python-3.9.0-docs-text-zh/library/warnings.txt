...
在 3.7 版更改: Previously "DeprecationWarning" and "FutureWarning"
were distinguished based on whether a feature was being removed
entirely or changing its behaviour. They are now distinguished based
on their intended audience and the way they're handled by the default
warnings filters.
...
在 3.7 版更改: "DeprecationWarning" is once again shown by default
when triggered directly by code in "__main__".

在 3.7 版更改: "BytesWarning" no longer appears in the default filter
list and is instead configured via "sys.warnoptions" when "-b" is
specified twice.
